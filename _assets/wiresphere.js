...
jQuery(function($) {
  ...
  function gradient() {
    function generateGradient() {
      ...
      canvas.height = $(BackgroundGradient)
        .parent()
        .height();
      canvas.width = $(BackgroundGradient)
        .parent()
        .width();
      ...
      var draw = function draw() {
        ...
        gradient.addColorStop(
          0,
          "rgba(" +
            WiresphereConfig.Background.Gradient.StopAColor +
            "," +
            alphaA +
            ")"
        );
        gradient.addColorStop(
          0.4,
          "rgba(" + WiresphereConfig.Background.Gradient.StopAColor + ",0)"
        );
        gradient.addColorStop(
          0.6,
          "rgba(" + WiresphereConfig.Background.Gradient.StopBColor + ",0)"
        );
        gradient.addColorStop(
          1,
          "rgba(" +
            WiresphereConfig.Background.Gradient.StopBColor +
            "," +
            alphaB +
            ")"
        );
  ...
  function menu() {
    ...
    $(Nav).on("click", "a", function() {
      if (!AnimationInProgress) {
        ...
        setTimeout(function() {
          $("#content-content--" + $(It).attr("data-page"))
            .css("display", "block")
            .animate(
              {
                opacity: 1
  ...
  function config() {
    $(BackgroundColor).css(
      "background",
      WiresphereConfig.Background.SolidBg.Color
    );
    $(BackgroundGradient).css(
      "opacity",
      WiresphereConfig.Background.Gradient.Opacity
    );

    if (WiresphereConfig.Background.Picture.Enabled) {
      $(BackgroundPic).css(
        "opacity",
        WiresphereConfig.Background.Picture.Opacity
      );
    } else {
    ...
    if (WiresphereConfig.Background.Noise.Enabled) {
      $(BackgroundNoise).css(
        "opacity",
        WiresphereConfig.Background.Noise.Opacity
      );
    } else {
    ...
    if (WiresphereConfig.Colors.JSOverride) {
      var CSS =
        '<style type="text/css">.stage > .stage-header h1 { color:' +
        WiresphereConfig.Colors.Tagline.FirstLine +
        ";} .stage > .stage-header h2 { color:" +
        WiresphereConfig.Colors.Tagline.SecondLine +
        ";} a.nice-link { color:" +
        WiresphereConfig.Colors.Buttons.Text +
        "; } a.nice-link > .hover > span { background:" +
        WiresphereConfig.Colors.Buttons.Outline +
        "; } a.nice-link:hover, a.nice-link.hover { color:" +
        WiresphereConfig.Colors.Buttons.TextHover +
        "; } .content { background:" +
        WiresphereConfig.Colors.SideContent.Background +
        "; color:" +
        WiresphereConfig.Colors.SideContent.Text +
        "; } .content .content-content.content-content--contact .contact-grid > li .fa { color:" +
        WiresphereConfig.Colors.SideContent.ContactIcons +
        "}</style>";
      $("head").append(CSS);
    }
  }
